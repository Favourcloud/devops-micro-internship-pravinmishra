...
Before each Claude session, set `DMI_TRIAGE_NOT_BEFORE` to that session’s UTC ISO timestamp. The guard rejects current-report reads without fresh evidence, including when an attempted wrapper command was denied. Use exactly `./run-triage.sh`, without redirects or added shell operators.
