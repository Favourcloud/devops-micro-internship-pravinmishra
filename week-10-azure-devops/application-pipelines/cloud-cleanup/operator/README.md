...
The original privilege deadline remains **19 September 2026, 18:18:03 UTC**. The root administration window was not extended. No role, issuer, VM or hosted cleanup service was created. **At this activation checkpoint, actual non-root sign-in, MFA-protected reads and Terraform-provider acceptance had not yet been verified**; attachment alone did not establish effective access. The shared AWS CLI 2.26.1 was below the documented 2.32.0 login prerequisite; subsequent isolated preparation and read-only acceptance are recorded separately below. This is not assignment completion or screenshot evidence.
...
## Verified non-root sign-in and read-only provider acceptance — 19 September 2026

After the owner reported private verification finished, independent CLI checks verified the exact **`dmi-week10-operator`** IAM identity in the approved account, the expected permissions boundary, and successful IAM reads protected by that boundary's unchanged MFA and original-time denies. Browser/Terminal authentication contents and credential-cache contents were not inspected.

A separate, data-only Terraform plan timestamped **00:20:01 UTC** was independently checked at **00:23:24 UTC**. Existing **Terraform 1.13.5 / AWS provider 6.64.0** successfully consumed the private CLI **2.36.49** `credential_process` bridge. Four data sources read the caller, user and two policies. The exact non-root caller/boundary and **both full live policy documents** matched frozen source `91787063e1e36bcdd08ab3d1811c50b9c7f73d62`. The plan had **zero managed resources, no applicable changes and no apply**; it neither reused nor changed the existing IAM state. Its SHA-256 was `090ef4ac99175521a6140107c7678ee299313ce1693d34bdb923495d1476c76a`.

The probe used cleared credential settings, the isolated login config/cache, and denied reads of shared AWS configuration. Credential-process JSON flowed only through the provider's private subprocess pipe, not terminal output, logs or Terraform variables. The ordinary private login cache remains sensitive. No root fallback, policy modification, provider download or infrastructure creation occurred. Existing providers were symlink-reused; invocation-owned short-path scratch was removed.

Native configuration validation passed with **zero errors/warnings** under external-network denial. **Four offline schema/drift tests passed**. The initial constant-only time precondition was corrected to reference fixed locals before live execution. A subsequent result-location assertion was corrected and tested against the same saved plan: unused data-only results are retained in refreshed `prior_state`, not `planned_values`. No second live plan or apply was needed; these tests are not themselves live evidence.

This verifies authentication and the observed MFA-protected reads, **not bootstrap writes, workload permissions, ongoing access or assignment completion**. Original privileges still expire **19 September 2026, 18:18:03 UTC**; a login session can expire sooner. The root window was not renewed. Independent cleanup/state custody, federation, budget/teardown approval and workload-capable permissions remain separate gates before any VM. Screenshots and learner reflections remain pending.
...
## Verified isolated CLI preparation — 18 September 2026

After explicit approval, AWS CLI **2.36.49** was extracted into a private session-owned directory. At **23:48:52 UTC**, its actual macOS/x86_64 executable passed the version gate and `login help`/`--remote` capability checks with network and filesystem writes denied. The shared `/usr/local/bin/aws` executable, resolved path and version **2.26.1** remained unchanged; no global installation, installer scripts, shared symlinks or shell-profile changes were performed.

The [official pinned macOS package](https://awscli.amazonaws.com/AWSCLIV2-2.36.49.pkg) was **59,937,151 bytes**, with observed SHA-256 `6ced2166d7299b2c503684431dbd50bddc279aab4beeb94efa5bbc220bc0e6b3` (a local download hash, not a claimed publisher checksum). Native macOS verification confirmed the trusted **AMZN Mobile LLC (94KV3E626L)** installer signature and Apple notarization before extraction. Extraction denied network access and confined writes to the private directory; archive paths and extracted links were checked. The downloaded installer was removed after validation; the private executable and provenance records were retained. See AWS's [installation](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) and [versioned-package](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-version.html) documentation.

An owner-only, isolated config/cache and a human-private sign-in helper were prepared. **Eight offline helper tests passed**, plus an actual noninteractive invocation refused authentication; network and filesystem writes were denied during these checks. These are local preparation/fixture results, **not a login, effective MFA-context check, Terraform-provider acceptance or assignment evidence**. No authenticated AWS call, root reuse, infrastructure operation or deadline renewal occurred in this CLI preparation. The protected IAM state remained unchanged. At this preparation checkpoint, human password/MFA/OAuth interaction was still pending; the later acceptance checkpoint is recorded above. Never inspect or publish login-cache contents.

## Private browser sign-in — human only, not executed by tests

AWS documents **2.32.0 or newer** for `aws login`. The approved private **2.36.49** executable now satisfies that local prerequisite; the shared default CLI **2.26.1** still does not. Use the verified private executable, not the default PATH command. On another machine, stop until an explicitly approved compliant CLI is available. Do not overwrite a shared installation or inspect existing HOME profiles/caches.
...
Direct SDK support for the `login_session` profile remains untested. The AWS-documented `credential_process` bridge below was verified with Terraform 1.13.5 / AWS provider 6.64.0 in the bounded read-only probe above. Use a separate `process` profile **only in the isolated config**, with the approved CLI's absolute quoted path:
...
Only the SDK may consume that command's credential JSON through its private subprocess pipe; **never run it standalone or print its output**. Any later separately authorized Terraform invocation must inherit the same isolated config/cache, a cleared credential environment and `AWS_PROFILE=process`; no fallback to HOME, metadata, static keys or the root broker. Recheck the exact identity and original expiry before each operation. The successful metadata probe does not establish effective permission for bootstrap writes or workloads; those remain acceptance gates. Do not use shell tracing, AWS `--debug`, `TF_LOG` or full-environment output.
