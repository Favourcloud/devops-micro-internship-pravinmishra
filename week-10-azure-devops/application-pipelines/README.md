...
The pipeline/helper files do **not** provision or configure a VM. Separate [Terraform web-target definitions](target/terraform/README.md) and a [human-operated Ansible target playbook, input validator and runbook](target/README.md) implement the infrastructure/configuration source below. Testing is **offline only**: shared Terraform guard and Azure mock plans passed; AWS provider schema/mock validation is still blocked by missing local tooling. Running these against approved accounts/hosts would create paid resources and change packages/account/Nginx/service configuration. Neither pipeline invokes them. Actual target provisioning/configuration and live validation remain pending.
