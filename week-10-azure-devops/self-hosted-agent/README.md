...
**Current image review:** the user has [attested full-size content/privacy review of all seven A1 screenshots](../evidence/a1-human-review-2026-09-19.json). This is a hash-bound user confirmation, not independently observed review, a secret-absence certification, learner reflection, PAT compliance or new runtime authorization. Other assignments' images are not covered.
...
A subsequent fresh authorized trial verified registration, an explicitly non-root `azdoagent` service, Online status and [successful manual run 1](https://dev.azure.com/aneneeze2021/DMI-Week10/_build/results?buildId=1&view=results) at **10:30:20 UTC on 18 September**. The actual required command output was checked. The service was removed, Terraform resource deletion was verified at **10:41:37 UTC**, and the agent was deregistered; **no live host or agent remains from this trial**. The [sanitized operational receipt](runtime-2026-09-18.json) records these facts, not screenshots or learner execution/reflections. PAT scope, expiry and revocation were unverified in that historical receipt. The [evidence manifest](evidence/manifest.json) now records genuine captures for **all seven slots**, from separate trials; screenshot 7 still shows historical run 1, not the later run 2. [Capture provenance](../evidence/README.md) distinguishes the retained pool and historical run from current health. A1 image review is now user-attested as noted above. The [checklist reconciliation](../evidence/README.md#checklist-reconciliation-from-existing-records) credits historical Tasks 2–6 against the existing records. Its notes item is now checked following [user approval of the assistant-drafted reflection](../evidence/a1-reflection-approval-2026-09-20.json); PAT compliance and the universal secrecy assertion remain unchecked. The shared PAT is left unchanged as requested. Local test success is not live evidence. The read-only candidate helper and manual-only verification YAML remain unchanged; neither performs provisioning or registration.

## Earlier bounded attempt — 19 September 2026

A separately authorized Ubuntu lab supplied genuine VM/public-IP and fresh SSH evidence for slots **2–3**. Package hash/version/library checks passed. Registration then timed out at the optional TFVC licence prompt **before any PAT was sent to the agent**. No new service, Online agent or pipeline run resulted; a corrected retry was not authorized within that retired window. The runbook below now explains declining that optional licence for Git-only work.
...
## Fresh human-direct retry — 19 September 2026

A new, explicitly approved 60-minute window began at **21:37:12 UTC**. The user reported completing the real hidden vendor prompt in a dedicated terminal and said “done”; input was not observed or retained, and the stored PAT was not forwarded to registration. Vendor exit 0 and independent configuration/API checks established agent **18**, `dmi-w10-a1-0919-8b281c`, in pool **11**. The non-root service and Online status were verified. Exactly one [manual run 2](https://dev.azure.com/aneneeze2021/DMI-Week10/_build/results?buildId=2&view=results) succeeded at **21:49:48 UTC**, with actual `uname -a`, `whoami` → `azdoagent`, and `df -h` output.

Original screenshots **4–6** were captured at **21:51–21:53 UTC** from separate fresh-proof terminals and the live pool page—not the registration terminal or replayed logs. Screenshot 4 is an allowlisted post-registration configuration read-back, not a transcript of token entry. On-device CPU-only OCR checked context and sensitive-screen markers; human visual/privacy review was pending at capture time and is now separately user-attested.

Early cleanup uninstalled the service at **21:56:09 UTC**, independently verified agent 18 removed at **21:56:13 UTC**, and verified empty Terraform state and exact cloud-resource absence at **21:59:38 UTC**, before expiry. The [distinct operational receipt](runtime-2026-09-19-interactive.json) and [capture provenance](../evidence/a1-interactive-2026-09-19.json) preserve this success without rewriting the earlier failure. No live host/agent remains from this retry. Full-access PAT reuse was explicitly approved unchanged; that runtime receipt did not claim least-privilege compliance, revocation, human image review or learner reflection. Image review has since received the separate user attestation above, and the assistant-drafted reflection has its own 20 September approval record. Neither rewrites this historical receipt or authorizes new operations. The run links are in a private Azure DevOps project and require authorized access. This authorization is retired.
...
The manifest preserves seven exact slot titles. All seven now link to original PNGs with verified hashes/times and `status: captured_user_review_attested`, bound to the separate user confirmation above. Native-terminal captures have no source URL. Paths and the manifest's attestation reference are relative to `week-10-azure-devops/`. The [original provenance](../evidence/captures-2026-09-19.json), [VM/SSH provenance](../evidence/a1-vm-ssh-2026-09-19.json) and [human-direct trial provenance](../evidence/a1-interactive-2026-09-19.json) retain their historical false human-review/completion flags and distinguish recorded output from current health. Tests verify image integrity, exact placement and unchanged original requirements. No tokens or fake output belong in the manifest. Do not fill slots with PNG placeholders, mock UI, or unreadable capture failures. Attributed technical notes describe verified operations; the separately user-approved reflection remains explicitly assistant-drafted, not independently observed learner execution.
