...
Original screenshots **4–6** were captured at **21:51–21:53 UTC** from separate fresh-proof terminals and the live pool page—not the registration terminal or replayed logs. Screenshot 4 is an allowlisted post-registration configuration read-back, not a transcript of token entry. On-device CPU-only OCR checked context and sensitive-screen markers; human visual/privacy review remains pending.

Early cleanup uninstalled the service at **21:56:09 UTC**, independently verified agent 18 removed at **21:56:13 UTC**, and verified empty Terraform state and exact cloud-resource absence at **21:59:38 UTC**, before expiry. The [distinct operational receipt](runtime-2026-09-19-interactive.json) and [capture provenance](../evidence/a1-interactive-2026-09-19.json) preserve this success without rewriting the earlier failure. No live host/agent remains from this retry. Full-access PAT reuse was explicitly approved unchanged; least-privilege compliance, revocation, human image review and learner reflection are not claimed. The run links are in a private Azure DevOps project and require authorized access. This authorization is retired.
