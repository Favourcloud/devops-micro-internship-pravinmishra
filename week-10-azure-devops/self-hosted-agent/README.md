...
The manifest preserves seven exact slot titles. Slots 1 and 7 now link to the original PNGs with verified hashes/times/source URLs and `status: captured_review_pending`; the other five remain `captured: false`, `status: pending`, with null metadata. Paths are relative to `week-10-azure-devops/`. The [shared provenance index](../evidence/captures-2026-09-19.json) retains false human-review/completion claims and distinguishes historical output from current health. Tests verify actual image integrity, exact slot placement and the unchanged original requirements. No tokens or fake output belong in the manifest. Do not fill slots with PNG placeholders, mock UI, or prior unreadable capture failures. Keep the learner's original Notes unanswered until they provide actual platform/org/pool details, issues and resolutions.
