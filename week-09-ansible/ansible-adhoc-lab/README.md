...
ansible-adhoc-lab/
├── .gitignore
├── README.md, validation.json, runtime-validation.json
├── terraform/
...
The coordinator approved this replacement after B1s allocation failed in the separate A4 lab. A2 later applied successfully after the A5 pilot and exact-plan approval. At preparation review, UK South Dldsv6-family and regional quotas were both 0/10 used; A2's eight cores plus A5's two left **no headroom**. D2lds_v6 had zone 2/3 restrictions but no location restriction; this configuration explicitly leaves `zone = null`. These historical quota/SKU checks are not current availability or a capacity reservation. A4's B1s-only requirement was not relaxed. A2 resources have now been deleted.
...
An empty state list plus approved read-only Azure checks for the dedicated resource group and lab tags should confirm no surviving VMs/disks/public IPs/network resources. The resource-group deletion guard refuses deletion if unexpected resources remain; investigate rather than disabling it or deleting the group manually. Verify deletion even after a failed apply; capture cleanup before marking complete. Do not delete the controller's existing key or agent. Only then retire obsolete local inventory/output files; retain protected state/receipts per the learner's policy. For the historical approved run, the coordinator completed separately reviewed saved-plan teardown: exactly 23 deletes, no creates/updates. Verification at **21:53:43 UTC on 16 September 2026** confirmed the resource group absent, each of four VMs/actual OS disks/public IPs NotFound, empty state-list and `{}` outputs, unchanged lineage and advanced empty state serial **48**. Original state snapshots, plans and raw receipts remain private. No live cleanup rerun is needed or authorized; zero charges are not claimed.
...
The genuine runtime blocker was in the **private controller environment**, not VM allocation or host-key trust: `ANSIBLE_CALLBACKS_ENABLED=''` becomes `['']` in Ansible core 2.21.4. The empty plugin name caused `ValueError: A non-empty plugin name is required` (exit 250) before ping tasks. No later ad-hoc or A3 tasks ran. After cleanup, a new private helper omitted that variable; the frozen original helper/failure logs remain untouched and no execution authority was copied. Five local tests passed: exact narrow change, real old callback failure, actual default callback loading with the variable absent, rejection of literal `[]` as an empty-list replacement, and genuine **localhost-only** pong. This does **not** fulfill managed-host ping or authorize a retry.

Technical takeaway: Terraform creates hosts and routing; inventory selects roles; Ansible depends on valid controller configuration as well as SSH/Python. `ping` is not ICMP and does not prove a website is healthy. These are AI-assisted engineering notes based on actual receipts, not invented learner experience. Firsthand reflection remains for the learner.
