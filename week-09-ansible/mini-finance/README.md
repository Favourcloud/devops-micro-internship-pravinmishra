...
The owner must obtain the new VM's SSH host public key/fingerprint via an independently trusted Azure console/administrator channel. They must compare it before adding the verified entry to project-local `ansible/.local/known_hosts` (mode `0600`). `ssh-keyscan` output alone is **not authenticated**; do not set `StrictHostKeyChecking=no`, `accept-new`, or trust a fingerprint solely because the same network supplied it. Stop if no trustworthy verification channel is available. This runbook does not execute Azure console commands.
