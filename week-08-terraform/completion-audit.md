...
3. **Assignment 04: EpicBook.** Follow the [runbook](terraform-aws-epicbook/README.md) and [24 September preflight](terraform-aws-epicbook/evidence/preflight-20260924.md). PR #53 updates Node to 22.23.3 and its official archive checksum; 22 mock runs and 65 Python checks passed. No A4 deployment has run. The prepared design includes 28 managed resource instances and an RDS database. Resolve the documented upstream checkout/order limitation before marking that checklist item complete; the current upstream supports cart evidence but not a completed checkout/order record.
