...
module "security" {
  ...
  gateway_enabled = var.public_endpoint_mode == "api_gateway"
}
module "load_balancing" {
  ...
  gateway_enabled                  = var.public_endpoint_mode == "api_gateway"
  gateway_bridge_security_group_id = module.security.gateway_bridge_id
...
module "gateway" {
  count             = var.public_endpoint_mode == "api_gateway" ? 1 : 0
  source            = "./modules/gateway"
  api_id            = var.gateway_api_id
  name              = var.name
  web_subnets       = values(module.network.web_subnets)
  security_group_id = module.security.gateway_id
  listener_arn      = module.load_balancing.gateway_listener_arn
}
