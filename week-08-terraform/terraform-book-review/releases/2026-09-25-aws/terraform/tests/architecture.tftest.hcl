...
run "reject_gateway_hostname_mismatch" {
  command = plan
  variables {
    public_endpoint_mode = "api_gateway"
    gateway_api_id       = "a1b2c3d4e5"
    public_hostname      = "a1b2d3c4e5.execute-api.eu-west-1.amazonaws.com"
    certificate_arn      = null
  }
  expect_failures = [var.gateway_api_id]
}
run "aws_managed_https_restricts_alb_to_private_link" {
  command = plan
  variables {
    public_endpoint_mode = "api_gateway"
    gateway_api_id       = "a1b2c3d4e5"
    public_hostname      = "a1b2c3d4e5.execute-api.eu-west-1.amazonaws.com"
    certificate_arn      = null
  }
  assert {
    condition     = module.load_balancing.controls.public_port == 80 && module.load_balancing.controls.public_protocol == "HTTP" && module.security.public_entry.cidr == null && module.security.public_entry.port == 80 && length(module.gateway) == 1 && module.load_balancing.controls.internal_private && module.load_balancing.controls.gateway_bridge_private && module.security.public_entry.source_group == module.security.gateway_bridge_id
    error_message = "AWS-provided HTTPS must terminate at API Gateway with a private-only VPC-link listener and no public HTTP ingress."
  }
}
