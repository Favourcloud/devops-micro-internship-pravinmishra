...
class SourceTests(unittest.TestCase):
    ...
    def test_source_and_dependency_release_locks_remain_distinct(self):
        original = upstream.load_lock()
        released = upstream.load_deployment_lock()
        expected = {"backend/package.json", "backend/package-lock.json", "frontend/package.json", "frontend/package-lock.json"}
        self.assertEqual(expected, set(upstream.RELEASE_PACKAGE_HASHES))
        changed = {p for p in original["files"] if original["files"][p] != released["files"][p]}
        self.assertEqual(expected, changed)
        self.assertEqual(set(original["files"]), set(released["files"]))
        self.assertEqual(upstream.RELEASE_ID, upstream.digest(json.dumps(upstream.RELEASE_PACKAGE_HASHES, sort_keys=True, separators=(",", ":")).encode()))
        self.assertNotIn("release_id", original)
        self.assertEqual(upstream.RELEASE_ID, released["release_id"])
...
class ArtifactTests(unittest.TestCase):
    ...
    def test_manifest_complete_unchanged_and_origin_bound(self):
        ...
        lock = {"release_id": upstream.RELEASE_ID, "files": {"notes.txt": upstream.digest(files["notes.txt"])}}
        manifest = {"schema_version": 2, "release_id": upstream.RELEASE_ID, "commit": upstream.COMMIT, "platform": "ubuntu-24.04-linux-amd64",
        ...
        with mock.patch.object(artifact, "load_lock", return_value=lock):
            ...
            for key, value in [("schema_version", 1), ("release_id", "wrong"), ("commit", "wrong"), ("public_origin", "http://wrong.invalid"),
            ...
            with self.assertRaisesRegex(Exception, "reviewed_release_source_modified"):
                verify(bundle())
