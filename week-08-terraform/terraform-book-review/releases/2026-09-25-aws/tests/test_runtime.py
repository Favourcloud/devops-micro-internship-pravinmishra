...
class ConfigAndSecretsTests(OfflineCase):
    ...
    def test_systemd_credential_readonly_mount_and_ordinary_file_isolation(self):
        import stat
        target = Path('/run/credentials/book-app.service/config')
        def metadata(path, **kwargs):
            return mock.Mock(st_uid=0, st_mode=(stat.S_IFREG | 0o440) if path == target else (stat.S_IFDIR | 0o755))
        with mock.patch.dict(os.environ, {'CREDENTIALS_DIRECTORY': str(target.parent)}), \
                mock.patch.object(Path, 'stat', metadata), \
                mock.patch.object(Path, 'is_symlink', return_value=False), \
                mock.patch.object(Path, 'is_file', return_value=True), \
                mock.patch.object(Path, 'read_text', return_value=json.dumps(config())):
            self.assertEqual(common.load_config(), config())
            with self.assertRaises(common.RuntimeFailure):
                common.load_config(target)  # Explicit ordinary-file path stays strict.
            with mock.patch.object(Path, 'is_symlink', return_value=True):
                with self.assertRaises(common.RuntimeFailure):
                    common.load_config()
            def writable_parent(path, **kwargs):
                m = metadata(path)
                if path == target.parent:
                    m.st_mode = stat.S_IFDIR | 0o775
                return m
            with mock.patch.object(Path, 'stat', writable_parent):
                with self.assertRaises(common.RuntimeFailure):
                    common.load_config()
            def non_root_file(path, **kwargs):
                m = metadata(path)
                if path == target:
                    m.st_uid = 999
                return m
            with mock.patch.object(Path, 'stat', non_root_file):
                with self.assertRaises(common.RuntimeFailure):
                    common.load_config()
