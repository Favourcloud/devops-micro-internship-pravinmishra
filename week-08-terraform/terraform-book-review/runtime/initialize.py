...
def initialize(config, master, app, connector=connect_db):
    ...
    try:
        ...
        # Without partial_revokes, MySQL database grants interpret underscores as wildcards.
        ...
        if not existing:
            with connection.cursor() as cursor:
                # Only the validated schema identifier enters SQL structure; account values are bound.
