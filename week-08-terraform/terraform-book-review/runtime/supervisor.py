...
def process_identity(pid, proc=Path("/proc")):
    # comm may contain spaces/parentheses; fields after its last ')' begin at field 3.
...
def lock_name(config):
    # Same schema uses the same lock through DNS failover; don't include AZ/instance/endpoint.
...
def terminate(child):
    if child is not None and child.poll() is None:
        # Popen starts a fresh process group. No name-based or unrelated process killing.
        try:
...
def start_once(config, secret, *, connector=connect_db, popen=subprocess.Popen,
    ...
    try:
        ...
        deadline = clock() + 180
        while clock() < deadline:
            # Lock loss and child death are never treated as a transient readiness failure.
            assert_lock(connection, name)
            if child.poll() is not None:
                raise RuntimeFailure("backend_exited")
            try:
