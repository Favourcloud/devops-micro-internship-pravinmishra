...
variable "public_hostname" {
  ...
  validation {
    condition     = can(regex("^([a-z0-9]([a-z0-9-]*[a-z0-9])?\\.)+[a-z]{2,63}$", var.public_hostname))
    error_message = "Provide a DNS hostname, not a URL, wildcard, IP, path or shell input."
  }
}
variable "certificate_arn" {
  type        = string
  description = "Existing valid same-region ACM certificate; this project creates no certificate or DNS records."
  validation {
    condition     = can(regex("^arn:aws:acm:[a-z0-9-]+:[0-9]{12}:certificate/[0-9a-f-]{36}$", var.certificate_arn)) && try(split(":", var.certificate_arn)[3] == var.region, false)
    error_message = "Provide a same-region ACM certificate ARN."
  }
}
