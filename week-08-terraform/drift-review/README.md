...
The [actual operational record](reports/live/operations.json) verifies a real baseline, an **unapplied** public-SSH configuration proposal, a clean technical reset, and deletion of only the newly created VPC/security group. Terraform/checker exits were 0/2/0; AWS cleanup verification at 18:25:59Z found empty states/inventory and exact-resource `NotFound` results. GitHub Copilot executed separately authorized operations, not Claude or a manually operating human. The user explicitly confirmed that [human resolution approval remains pending](reports/live/human-resolution.json); autonomous cleanup and general continuation permission do not complete that requirement. Previous fixture reports remain synthetic. [Historical Claude runtime attempts](reports/live/claude-runtime.json) failed with Marketplace enrollment authorization errors; those failures are preserved, not rewritten. Enrollment availability was subsequently verified on 16 September, but no successful Claude review or runtime hook execution is established by that availability check. Earlier coursework and personal settings were unchanged by the isolated lab. Publication remains pending.

The [16 September continuation record](enrollment-proposal/SETUP-20260916.md) and [sanitized evidence](reports/continuation-20260916.json) now verify private MFA authentication, the user's local SDK offer acceptance at **13:26:22 UTC**, and independent `AVAILABLE / AUTHORIZED / AVAILABLE / AVAILABLE` readiness at **13:27:31 UTC**. The temporary session expired at **13:29 UTC** and the unchanged policy window at **13:30 UTC (14:30 Lagos)**. No renewal or identity/access-key removal is claimed. The user separately approved **up to $0.32 of the existing allowance**, preserving the $0.18 reservation; this is not provisioning or human Terraform-resolution approval.
...
Claude Code's earlier tool-free connection through restricted Bedrock Haiku (`global.anthropic.claude-haiku-4-5-20251001-v1:0`, origin ap-south-1, global routing) returned `BEDROCK_CONNECTION_OK`: 120 input/10 output tokens, reported $0.00017. The user separately approved up to **$0.50 additional usage**. Subsequent attempts found an unknown Skill, then fixed discovery, but hit HTTP 403 Marketplace enrollment authorization. No model review, Read call or runtime hook event succeeded in those attempts. At the **16 September 13:30:22.429 UTC** approval, additional reported cost remained $0; **$0.18 stayed reserved for incomplete usage records, not verified charges**, and the user authorized up to the remaining **$0.32**, not another allowance. A CLI threshold is not an AWS account cap. OmniRoute and personal settings were not changed.

[AWS documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html) explains that initial invocations can temporarily succeed during automatic enrollment (up to 15 minutes), then fail if prerequisites are missing. The original error did not identify the exact prerequisite. The later actual agreement acceptance and availability response, not the old connection test, establish the recorded enrollment milestone. The restricted inference role does **not** need Marketplace permissions after enrollment; it was not broadened. Do not use root, switch models or resubmit billed tests as a workaround. Account credentials/MFA and license acceptance remain private human actions.

The [Bedrock continuation runbook](BEDROCK_ACCESS.md) separates the historical administrator handoff from completed enrollment. AWS CLI 2.26.1 lacked the agreement commands; the user used a guarded local SDK helper instead. The separate restricted runtime role subsequently passed identity-only verification, not inference. Installed Claude Code 2.1.220 and the [native hook reference](https://code.claude.com/docs/en/hooks#exec-form-and-shell-form) support the existing `command` plus `args` exec form. The configured-command regression uses synthetic JSON, including paths with spaces; it does not prove native runtime enforcement. Preserve all reservations and verify durable shared accounting before the next bounded call.

A **13:44:17 UTC** read-only preflight verified empty isolated states and zero matching tagged VPCs/security groups. The fresh saved network plan proposes **one VPC create, no updates or deletions**, and remains **unapplied**. The closed security group's fresh create plan must follow a newly authorized VPC and use its actual binding, never the deleted VPC ID. Fresh provisioning/cleanup and the later human resolution decision are separate approval gates. All five pending screenshot slots remain unchanged.
