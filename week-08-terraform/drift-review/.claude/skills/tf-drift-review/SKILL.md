...
3. For an authorized live review, Read only the requested sanitized report: `reports/live/baseline-report.txt`, `reports/live/drift-detected-report.txt`, or `reports/live/resolved-report.txt`. Check that it identifies `Mode: LIVE`, its timestamp, source hash and actual Terraform detailed exit. Do not read raw plans, state, bindings, credentials or logs. These reports are generated outside the Skill by the separately authorized agent; reading them does not authorize Terraform or cloud commands. A proposed configuration change is not out-of-band drift. Alternatively inspect `reports/drift-detected-report.txt` or `reports/resolved-report.txt` as historical **synthetic fixtures only**, never as a live baseline.
